...
export default asWritingTask(async function cleanupOldFilesTask({ application, control }) {
  ...
  await control.cleanupFiles({
    '8.10.1': [
      `${application.clientSrcDir}app/account/login.service.ts`,
      [
        (application.authenticationTypeSession && application.generateUserManagement) || application.authenticationTypeOauth2,
        `${application.clientSrcDir}app/account/login.service.spec.ts`,
      ],
    ],
  });
});
