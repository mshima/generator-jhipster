      ...
      npmScripts({ application }) {
        ...
        Object.assign(application.packageJsonScripts, {
          ...
          'e2e:devserver': application.clientFrameworkAngular
            ? `concurrently -k -s first -n backend,e2e -c red,blue npm:backend:start "npm run ci:server:await && ng run ${application.dasherizedBaseName}:cypress-run${application.cypressCoverage ? ':coverage' : ''}"`
