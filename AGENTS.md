...
JHipster is a Yeoman-based code generator (TypeScript, ESM, Node — see `engines` in `package.json` for the required version) that scaffolds Spring Boot + Angular/React/Vue applications. See `ARCHITECTURE.md`, `BLUEPRINTS.md`, and `DEVELOPMENT.md` for background context. Deeper, verified implementation notes for agents live in `AGENTS-KNOWLEDGE.md` — read it before digging into built-in entities, user-management, translation macros, user caches, or sample generation.
