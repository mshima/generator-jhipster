...
if [[ "$JHI_ENTITY" == "jdl" ]]; then
    ...
    eval -V "$JHI_CLI import-jdl *.jdl --no-insight --skip-jhipster-dependencies $@"
    ...
    cd "$JHI_FOLDER_APP"
    eval -V "$JHI_CLI --force --no-insight --skip-checks --with-entities --skip-jhipster-dependencies $@"
