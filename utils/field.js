...
function prepareFieldForTemplates(entityWithConfig, field, generator) {
  ...
  if (field.id) {
    ...
    if (!field.autoGenerate) {
      autoGenerateData = { liquibaseAutoIncrement: false, jpaGeneratedValue: false };
    } else if (entityWithConfig.reactive) {
      autoGenerateData = { liquibaseAutoIncrement: true, jpaGeneratedValue: false };
    } else if (entityWithConfig.prodDatabaseType === 'mysql' && field.fieldType === 'Long') {
      autoGenerateData = { liquibaseAutoIncrement: true, jpaGeneratedValue: 'identity' };
    } else if (entityWithConfig.databaseType !== 'sql') {
      autoGenerateData = { liquibaseAutoIncrement: false, jpaGeneratedValue: false };
    } else {
      autoGenerateData = {
        liquibaseAutoIncrement: false,
        jpaGeneratedValue: field.fieldType === 'Long' ? 'sequence' : field.fieldType === 'UUID',
      };
    }
    _.defaults(field, autoGenerateData);
    _.defaults(field, { readonly: !!field.jpaGeneratedValue || field.liquibaseAutoIncrement });
  }
